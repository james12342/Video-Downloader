...
//video text
var VideoAddText = document.getElementsByClassName('VideoAddText')[0];
...
const orderReccentFiles = (dir) => {
	return fs1.readdirSync(dir)
		...
		.map((file) => ({
			file,
			mtime: fs.lstatSync(path1.join(dir, file)).mtime
		}))
...
VideoAddText.onclick = function () {
//text on video

  var AddvideoText = document.getElementsByClassName('AddvideoText')[0].value;
 
  if(AddvideoText.length==0)
  {
   alert("Please enter the text you want to put into video,otherwise it will be @J AI Media");
   AddvideoText='@J AI Media';

  }

	var latestFileName = getMostRecentFile('./download/');
  alert('Please wait,You are going to add text:'+AddvideoText+ 'into Video:'+latestFileName);
	//alert('please wait generating text on video:' + latestFileName);
	//var textonVideo=`ffmpeg -i `+`${videoDownloadFullPath}`+'/'+`${latestFileName}`+` -vf "drawtext=fontfile=/path/to/font.ttf:text='Stack Overflow':fontcolor=white:fontsize=24:box=1:boxcolor=black@0.5:boxborderw=5:x=(w-text_w)/2:y=(h-text_h)/2" -codec:a copy `+`${videoDownloadFullPath}/TEXT_`+`${latestFileName}`;
	//var textonVideo=`ffmpeg -i `+`${videoDownloadFullPath}`+'/'+`${latestFileName}`+` -vf "drawtext=text='James AI Technology': x=100: y=h-80*t: fontsize=32:fontcolor=yellow@0.9: box=1: boxcolor=black@0.6" -codec:a copy `+`${videoDownloadFullPath}/TEXT_`+`${latestFileName}`;
	var textonVideo = `ffmpeg -i ` + `${videoDownloadFullPath}` + '/' + `${latestFileName}` + ` -vf "drawtext=text='`+ `${AddvideoText}` +`': y=h-line_h-10:x=w-(t-1)*w/30: fontsize=32:fontcolor=yellow@0.9: box=1: boxcolor=black@0.6" -codec:a copy ` + `${videoDownloadFullPath}/TEXT_` + `${latestFileName}`;
	const l_textonV = exec(textonVideo, (error, stdout, stderr) => {
		alert(textonVideo);
		if (error) {
			console.error(`exec error: ${error}`);
			alert(`${error}`);
		...
		l_textonV.stdout.on('data', data => {
			percentage.innerText = data;

			console.log(`stdout: ${data}`);
		});
		l_textonV.stdio.on('data', data => {
			percentage.innerText = data;

			console.log(`stdio: ${data}`);
		});
		l_textonV.stderr.on('data', data => {
			percentage.innerText = data;
